...
export default function DashboardIntegrated() {
  ...
  const loadOffices = async (location) => {
    try {
      setIsLoading(true);
  ...
  const loadFloors = async (location, office) => {
    try {
      setIsLoading(true);
  ...
  const handleStepSelection = async (stepIndex, value) => {
    switch (stepIndex) {
      case 0:
  ...
  const getRealtimeStatus = (machineId) => {
